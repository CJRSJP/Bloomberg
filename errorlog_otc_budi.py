...
def process_file_errors(bucket_name, file_list, cs):
    ...
    for file in file_list:
        ...
        cs.execute(f"SELECT COUNT(*) FROM IM_REC.CR_EPHEMERAL.OTC_BUDI_ERRORS WHERE FILE_NAME = %s", (key.replace('Extracts/IDH003/ErrorLogs/', ''),))
        ...
        try:
            ...
            file_df = pd.read_csv(io.StringIO(file_data), names=['ID', 'ACTION', 'STATUS', 'BLOOMBERG_ID', 'BLOOMBERG_ID_TYPE', 'XREF_CONTEXT', 'ERROR_MSG'])
            ...
            for _, row in file_df.iterrows():
                if row['STATUS'].strip().upper() == 'ERROR':
                    error_records.append({
                            ...
                            'ACTION': row['ACTION'],
                            'STATUS': row['STATUS'],
                            'BLOOMBERG_ID': row['BLOOMBERG_ID'],
                            'BLOOMBERG_ID_TYPE': row['BLOOMBERG_ID_TYPE'],
                            'XREF_CONTEXT': row['XREF_CONTEXT'],
    ...
    error_df = pd.DataFrame(error_records, columns=['FILE_NAME', 'ID', 'ACTION', 'STATUS', 'BLOOMBERG_ID', 'BLOOMBERG_ID_TYPE', 'XREF_CONTEXT', 'ERROR_MSG', 'LOAD_DATE_TIME'])
